...
/**
 * Initializes the Aes struct and expands the key.
 * 
 * @param aes the AES structure
...
init(Aes *aes, const uint8_t *key, size_t key_length)
{
    ...
    for (i = key_length; i < key_schedule_size; i++)
    {
        ...
        aes->encryption_scheduled_key[i] ^= aes->encryption_scheduled_key[i - key_length];
    }
    ...
    for (i = 0; i < key_schedule_size; i++)
    {
        ...
        else
        {
            ...
            temp = td[sbox[(aes->encryption_scheduled_key[i] >> 16) & 0xFF]];
            aes->decryption_scheduled_key[i] ^= rol_32(temp, 16);
            temp = td[sbox[(aes->encryption_scheduled_key[i] >> 24) & 0xFF]];
            aes->decryption_scheduled_key[i] ^= rol_32(temp, 24);
        }
    }
...
/**
 * Encrypts a 128-bit block of a given input pointer and assigns
 * the cipher text to an output pointer.
 * 
 * @param aes the AES structure
...
encrypt(Aes *aes, const uint8_t *input, uint8_t *output)
{
    ...
    s1 ^= aes->encryption_scheduled_key[1];
    s2 ^= aes->encryption_scheduled_key[2];
    ...
    for (i = 1; i < aes->number_of_rounds; i++)
    {
        ...
        s1 = t1 ^ aes->encryption_scheduled_key[i * 4 + 1];
        s2 = t2 ^ aes->encryption_scheduled_key[i * 4 + 2];
        s3 = t3 ^ aes->encryption_scheduled_key[i * 4 + 3];
    }
    ...
    s0 = t0 ^ aes->encryption_scheduled_key[aes->number_of_rounds * 4];
    s1 = t1 ^ aes->encryption_scheduled_key[aes->number_of_rounds * 4 + 1];
    s2 = t2 ^ aes->encryption_scheduled_key[aes->number_of_rounds * 4 + 2];
...
/**
 * Decrypts a given 128-bit block cipher text input and assigns
 * the result to a plaintext output pointer.
 * 
 * @param aes the AES structure
...
decrypt(Aes *aes, const uint8_t *input, uint8_t *output)
{
    ...
    s1 ^= aes->decryption_scheduled_key[aes->number_of_rounds * 4 + 1];
    s2 ^= aes->decryption_scheduled_key[aes->number_of_rounds * 4 + 2];
    ...
    for (i = aes->number_of_rounds - 1; i >= 1; i--)
    {
        ...
        s1 = t1 ^ aes->decryption_scheduled_key[i * 4 + 1];
        s2 = t2 ^ aes->decryption_scheduled_key[i * 4 + 2];
        s3 = t3 ^ aes->decryption_scheduled_key[i * 4 + 3];
    }
    ...
    // Last round key addition
    s0 = t0 ^ aes->decryption_scheduled_key[0];
    s1 = t1 ^ aes->decryption_scheduled_key[1];
    s2 = t2 ^ aes->decryption_scheduled_key[2];
